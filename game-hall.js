...
document.addEventListener("DOMContentLoaded", () => {
	...
	async function openWerewolfSetup() {
		...
		let playerOptions = [...singleChats.map((c) => ({ id: c.id, name: c.name, avatar: c.settings.aiAvatar, type: "角色" })), ...allNpcs.map((n) => ({ id: n.id, name: n.name, avatar: n.avatar, type: `NPC (${n.owner})` }))];
			...
			case "wolf_action":
				...
				if (userPlayer) {
					...
					if (aiWolves.length > 0) {
						// 从AI队友获取建议
						const aiVotePromises = aiWolves.map((wolf) => triggerWerewolfAiAction(wolf.id, "wolf_kill", { isUserWolfAlly: true }));
				...
				if (tied && maxVotes > 0) {
					...
					logToWerewolfGame(`(狼人内部经过一番激烈讨论，最终决定目标为 ${werewolfGameState.players.find((p) => p.id === targetId).name})`);
				}
			...
			case "day_start":
				...
				werewolfGameState.lastNightKilled.forEach((killedId) => {
					if (killedId === werewolfGameState.guardLastNightProtected) {
						logToWerewolfGame(`昨晚 ${werewolfGameState.players.find((p) => p.id === killedId).name} 被袭击但同时也被守护了。`);
					} else {
	...
	async function handleWerewolfReroll(logIndex) {
		...
		// 给用户一个即时反馈
		const speechTextElement = document.querySelector(`button[data-log-index="${logIndex}"]`).closest(".speech-content").querySelector(".speech-text");
...
`;
	...
	async function startSeaTurtleSoupGame() {
		...
		logToStsGame(`游戏开始！出题人是 ${seaTurtleSoupState.riddleProvider.name}。`, "system", seaTurtleSoupState.riddleProvider);
...
`;
	...
	function renderSeaTurtleGameScreen(options = {}) {
		...
		seaTurtleSoupState.players.forEach((player) => {
			const seat = document.createElement("div");
			seat.className = "player-seat";
			const roleIndicator = player.isProvider ? '<div class="player-role-indicator riddle-master" title="出题人">👑</div>' : "";
	...
	async function handleStsUserGuess() {
		...
		if (provider.isUser) {
			isCorrect = await showCustomConfirm("判断猜测", `玩家 ${userPlayer.name} 猜测的答案是：\n\n"${guess}"\n\n这个猜测是否正确？`);
		} else {
	...
	async function processStsTurn(userQuestion = null, userObj = null) {
		...
		for (const guesser of guessers) {
			...
			if (aiAction.type === "question") {
				...
				let isCorrect = false;
				if (seaTurtleSoupState.riddleProvider.isUser) {
					isCorrect = await showCustomConfirm("判断猜测", `玩家 ${guesser.name} 猜测的答案是：\n\n"${aiAction.content}"\n\n这个猜测是否正确？`);
				} else {
...
现在，请根据你的人设和判断，生成你的行动。`;
	...
	async function startScriptKillGame() {
		...
		hideCustomModal();
		await showCustomAlert(`你的角色是：【${myPlayer.role.name}】`, `**角色介绍:**\n${myPlayer.role.description}\n\n**你的任务:**\n${myPlayer.role.tasks}`);
		...
		switch (scriptKillGameState.gamePhase) {
			...
			case "introduction":
				...
				for (const player of scriptKillGameState.players) {
					...
					let introduction = player.isUser ? await waitForUserActionSK("轮到你自我介绍了", "speak", "简单介绍一下你自己和你所扮演的角色...") : await triggerScriptKillAiAction(player.id, "introduce");
			...
			case "timeline_discussion":
				...
				for (const player of scriptKillGameState.players) {
					renderScriptKillGameScreen({ speakingPlayerId: player.id });
					let timelineSpeech = player.isUser ? await waitForUserActionSK("轮到你陈述时间线了", "speak", "请根据你的剧本，详细说明你的行动轨迹...") : await triggerScriptKillAiAction(player.id, "discuss_timeline");
			...
			case "voting":
				...
				for (const voter of alivePlayers) {
					let targetId = voter.isUser ? await waitForUserActionSK("请投票指认凶手", "vote") : await triggerScriptKillAiAction(voter.id, "vote");
	...
	async function handleScriptKillReroll(logIndex) {
		...
		// 给用户一个即时反馈
		const speechTextElement = document.querySelector(`button.sk-reroll-btn[data-log-index="${logIndex}"]`).closest(".speech-content").querySelector(".speech-text");
		if (speechTextElement) {
			speechTextElement.innerHTML = "<i>正在重新思考...</i>";
		}
...
`;
		...
		switch (action) {
			...
			case "discuss_timeline":
				systemPrompt += "现在是时间线陈述环节。请根据你的角色剧本（包括身份设定和秘密任务），详细、清晰地陈述你在案发时间段内的行动轨迹。你的发言必须是第一人称，并且要听起来自然，可以适当隐藏对你不利的信息，但不能凭空捏造。";
	...
	async function renderScriptManagerList() {
		...
		if (scripts.length === 0) {
			listEl.innerHTML = '<p style="text-align:center; color: var(--text-secondary); padding: 50px 0;">还没有自定义剧本，点击右上角“添加”创建一个吧！</p>';
			return;
		}
...
`;
	...
	async function startGuessWhatGame() {
		...
		if (gameMode === "ai_guesses") {
			...
			logToGuessWhatGame({ player: opponentInfo, text: `【${opponentInfo.name}托着下巴想了想】第一个提示是... ${firstHint}` }, "ai-turn");
	...
	async function handleGuessWhatReroll(logIndex) {
		...
		// 安全检查
		if (userLogIndex < 0 || !guessWhatGameState.gameLog[userLogIndex] || guessWhatGameState.gameLog[userLogIndex].type !== "user-turn") {
	...
	async function processGuessWhatTurn(userInput) {
		...
		// 3. 让AI根据上下文执行动作
		const aiResponse = await triggerGuessWhatAiAction(currentMode === "ai_guesses" ? "guess_word" : "give_hint", userInput);
	...
	async function triggerGuessWhatAiAction(actionType, userInput = null) {
		...
		const historyText = guessWhatGameState.gameLog
			.map((log) => (log.type === "system" ? `[系统提示: ${log.message}]` : `${log.message.player.name}: ${log.message.text}`))
			.slice(-10)
			.join("\n");
		let systemPrompt = `# 你的任务\n你正在扮演角色“${opponent.name}”，人设是：“${opponent.persona}”。\n你正在和“${state.qzoneSettings.nickname || "我"}”玩“你说我猜”游戏。\n你的所有发言都【必须】严格符合你的人设和口吻，让整个过程像一次真实的聊天互动。\n\n# 游戏历史 (最近的对话)\n${historyText}\n`;
		switch (actionType) {
			case "generate_word":
...
{"type": "guess", "text": "【假装恍-然大悟】哦~我知道了，是“电饭煲”对不对？...好吧好吧不逗你了，我猜是...[你的真实猜测]"}`;
	...
	async function handleLudoQuestionEvent(player) {
		...
		// 5. 根据选择的模式，执行不同的流程
		logToLudoGame(`【${mode === "both_answer" ? "共同回答" : "一人回答，一人评价"}】抽到的问题是：“${questionText}”`, "system");
		await sleep(1500);
		...
		if (mode === "both_answer") {
			...
			let answer1 = currentPlayer.isUser ? await waitForLudoUserAction("轮到你回答问题", "请输入你的回答...") : await triggerLudoAiAction("answer_question", { question: questionText });
			...
			let answer2 = otherPlayer.isUser ? await waitForLudoUserAction("轮到你回答问题", "请输入你的回答...") : await triggerLudoAiAction("answer_question", { question: questionText });
			logToLudoGame(`<strong>${otherPlayer.name}:</strong> ${answer2}`, otherPlayer.isUser ? "user" : "char");
		} else if (mode === "single_answer") {
			logToLudoGame(`请 <strong>${currentPlayer.name}</strong> 先回答。`, "system");
			let answer = currentPlayer.isUser ? await waitForLudoUserAction("轮到你回答问题", "请输入你的回答...") : await triggerLudoAiAction("answer_question", { question: questionText });
			...
			let evaluation = otherPlayer.isUser ? await waitForLudoUserAction(`对“${answer}”的看法`, "请输入你的评价...") : await triggerLudoAiAction("evaluate_answer", { question: questionText, answer: answer });
	...
	async function handlePlayerMove(player, diceRoll, isUserMove) {
		logToLudoGame(`<strong>${player.name}</strong> 掷出了 <strong>${diceRoll}</strong> 点！`, isUserMove ? "user" : "char");
...
`;
	...
	async function generateLudoBoard() {
		...
		if (questionsInBank.length > availableCellIndices.length) {
			console.warn(`飞行棋警告：问题库中的问题数量(${questionsInBank.length})超过了棋盘上的可用格子数量(${availableCellIndices.length})，部分问题将不会出现。`);
		}
	...
	async function renderLudoQuestionBanks() {
		...
		if (banks.length === 0) {
			listEl.innerHTML = '<p style="text-align:center; color: var(--text-secondary); padding: 50px 0;">还没有问题库，点击右上角“新建”创建一个吧！</p>';
		} else {
			banks.forEach((bank) => {
				const item = document.createElement("div");
				...
				addLongPressListener(item, async () => {
					...
					if (choice === "rename") {
						...
						const confirmed = await showCustomConfirm("确认删除", `确定要删除问题库“${bank.name}”吗？这将同时删除库内所有问题。`, { confirmButtonClass: "btn-danger" });
						if (confirmed) {
	...
	async function renderLudoQuestionsInBank(bankId) {
		...
		if (questions.length === 0) {
			listEl.innerHTML = '<p style="text-align:center; color: var(--text-secondary); padding: 50px 0;">这个题库还是空的，点击右上角“+”添加第一个问题吧！</p>';
		} else {
			questions.forEach((q) => {
				const item = document.createElement("div");
				item.className = "list-item";
	...
	async function openUndercoverSetup() {
		...
		if (playerOptions.length < 2) {
			selectionEl.innerHTML = '<p style="text-align:center; padding-top: 50px; color: var(--text-secondary);">你需要至少2位AI或NPC好友才能开始游戏哦。</p>';
	...
	async function processUndercoverTurn() {
		...
		switch (undercoverGameState.gamePhase) {
			...
			case "voting_round": {
				...
				for (const voter of alivePlayers) {
					...
					if (voteResult && voteResult.voteForId) {
						...
						if (targetPlayer) {
							logToUndercoverGame(`<strong>${voter.name}</strong> 投票给了 <strong>${targetPlayer.name}</strong>，理由是：“${reason}”`);
			...
			case "elimination": {
				...
				if (playersToEliminate.length > 1) {
					// 处理平票
					logToUndercoverGame(`出现平票: ${playersToEliminate.map((id) => undercoverGameState.players.find((p) => p.id === id).name).join("、 ")}。`, "system");
			...
			case "tie_vote_re-vote": {
				...
				const voters = undercoverGameState.players.filter((p) => p.isAlive && !undercoverGameState.tiedPlayers.includes(p.id));
	...
	async function handleUndercoverReroll(logIndex) {
		...
		const speechTextElement = document.querySelector(`button.uc-reroll-btn[data-log-index="${logIndex}"]`).closest(".speech-content").querySelector(".speech-text");
		if (speechTextElement) {
	...
	function waitForUserUndercoverAction(promptText, actionType, context = {}) {
		...
		return new Promise((resolve) => {
			...
			if (actionType === "speak") {
				...
				const targets = context.targets ? context.targets.filter((p) => !p.isUser) : undercoverGameState.players.filter((p) => p.isAlive && !p.isUser); // 否则是所有存活玩家
	...
	async function triggerUndercoverAiAction(playerId, actionType, context = {}) {
		...
		switch (actionType) {
			...
			// =======================================================
			case "tie_speak":
				actionPrompt = "你因为平票正在进行补充发言。请为自己辩解，说服其他人不要投票给你。你的发言要简短有力，符合你的人设和身份。";
...
`;
	...
	function openUndercoverSummaryTargetPicker(summaryText) {
		...
		confirmBtn.parentNode.replaceChild(newConfirmBtn, confirmBtn);
		newConfirmBtn.onclick = () => {
			const selectedIds = Array.from(document.querySelectorAll(".undercover-target-checkbox:checked")).map((cb) => cb.value);
			if (selectedIds.length > 0) {
